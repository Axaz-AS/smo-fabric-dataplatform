create view [int].[int_ops__patient_unified] as /*
...
WITH pasient_detail AS (
    SELECT * FROM [wh_silver].[int].[base_nobs__statpasient_detail]
),

pasient AS (
    SELECT * FROM [wh_silver].[int].[base_nobs__exp_pasient]
),

personalia AS (
    SELECT * FROM [wh_silver].[int].[base_nobs__exp_personalia]
),
